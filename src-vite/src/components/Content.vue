<template>
  <div
    class="relative flex-1 flex flex-col select-none"
    :class="{
      'opacity-50 pointer-events-none': uiStore.isInputActive('AlbumList-edit'),
    }"
  ...
  >
    ...
    >
      <!-- title -->
      <div
        class="mr-1 flex flex-row items-center gap-1 min-w-0 flex-1 overflow-hidden"
        data-tauri-drag-region
      >
        <component
          v-if="currentTitleIcon"
          :is="currentTitleIcon"
        ...
        <div class="overflow-hidden min-w-0 flex-1">
          <div
            v-if="contentTitle"
            class="breadcrumbs p-0 min-h-0 overflow-hidden"
          >
            <ul
            ...
            >
              <li
                v-for="(seg, idx) in titleSegments"
                :key="idx"
      ...
      <div class="flex items-center gap-2 shrink-0">
        ...
        <div class="flex flex-row items-center">
          ...
          <TButton
            :icon="
              [IconCard, IconTile, IconJustified, IconJustified][
                config.settings.grid.style
              ]
            "
            :iconStyle="{
            ...
            :tooltip="
              localeMsg.settings.view.style_options[config.settings.grid.style]
            "
          ...
          <TButton
            ...
            :tooltip="
              isInfoPanelOpen
                ? $t('toolbar.tooltip.hide_info')
                : $t('toolbar.tooltip.show_info')
            "
    ...
    <div
      ref="contentViewDiv"
      class="relative flex-1 flex flex-row overflow-hidden"
    >
      <div class="relative flex-1 flex flex-row overflow-hidden">
        <div
          ...
          :class="[
            'flex-1 flex',
            gridViewLayoutClass,
            config.settings.grid.showFilmStrip && !showWelcomeContent
        ...
        >
          <div
            ...
            :class="{
              'flex-1':
                showWelcomeContent || !config.settings.grid.showFilmStrip,
            }"
            :style="{
              height:
              ...
              >
                <button
                  :class="[
                    ...
                    selectedItemIndex > 0
                      ? 'text-base-content/70 hover:text-base-content hover:bg-base-100/70 cursor-pointer'
                ...
                >
                  <component
                    :is="isFilmstripVertical ? IconArrowUp : IconLeft"
                    class="w-8 h-8"
                  />
                </button>
                <button
                ...
                >
                  <component
                    :is="isFilmstripVertical ? IconArrowDown : IconRight"
                    class="w-8 h-8"
                  />
                </button>
              </div>
          ...
          <div
            v-if="!showWelcomeContent && config.settings.grid.showFilmStrip"
          ...
          >
            <div
              v-if="
                selectedItemIndex >= 0 && selectedItemIndex < fileList.length
              "
        ...
        <!-- Quick View Overlay -->
        <div
          v-if="showQuickView && fileList[selectedItemIndex]"
      ...
      <div
        ...
        :style="{
          width: rightPanelLayoutVisible ? activeRightPanelWidth + 'px' : '0px',
        }"
      >
        <div
          :class="[
            ...
            rightPanelVisualVisible
              ? 'translate-x-0'
              : 'translate-x-full pointer-events-none',
          ]"
          :style="{
            width: activeRightPanelWidth + 'px',
            top: '48px',
            bottom: config.settings.showStatusBar ? '32px' : '4px',
          }"
        >
          <DedupPane
    ...
    <StatusBar
      ...
      :selected-file="fileList[selectedItemIndex]"
      :show-scale="true"
      :is-embedded="false"
      ...
      :enrich-total="
        galleryStore.enrichingActive + galleryStore.enrichingQueued
      "
      :compare-stack-size="galleryStore.compareStack.length"
    />
  </div>
  ...
  <MessageBox
    ...
    :message="
      $t('msgbox.unsaved_changes.message') ||
...
import {
  ...
  importFileBytes,
  addFileToDb,
  cancelIndexing as cancelIndexingApi,
  selectFolder,
  getFacesForFile,
  listenFaceIndexProgress,
  openFileWithApp,
  getAppConfig,
  getIndexRecoveryInfo,
  clearIndexRecoveryInfo,
  setLastSelectedItemIndex,
  dedupGetGroup,
  dedupDeleteSelected,
  getQueryFilePosition,
  getFolderSearchExcluded,
} from "@/common/api";
...
import {
  getSmartTagById,
  SMART_TAG_SEARCH_THRESHOLD,
} from "@/common/smartTags";
import {
  getAlbumScanState,
  getAlbumScanIcon,
  shouldAnimateAlbumScanIcon,
} from "@/common/scanStatus";
import {
  isWin,
  isMac,
  setTheme,
...
const thumbnailPlaceholder = new URL(
...
function handleBreadcrumbClick(segmentIndex: number) {
  if (tempViewMode.value === "album") {
    ...
    const targetPath = pathParts
      .slice(0, Math.max(0, pathParts.length - levelsToGoUp))
      .join(separator);
  ...
  if (sidebarIndex === 0 || sidebarIndex === 1) {
    ...
    const levelsToGoUp = segments.length - 1 - segmentIndex;
    const targetPath = pathParts
      .slice(0, pathParts.length - levelsToGoUp)
      .join(separator);
    ...
    if (sidebarIndex === 0) {
      ...
      contentTitle.value = segments.slice(0, segmentIndex + 1).join(" > ");
      tauriEmit("expand-album-folder", {
        albumId: libConfig.album.id,
...
const selectionChunkSize = computed(
  () => Number(config.main?.selectionChunkSize) || 200,
);
const selectionMaxFiles = computed(
  () => Number(config.main?.selectionMaxFiles) || 400,
);
const isRealFileItem = (item: any) =>
  !!item && !item.isPlaceholder && typeof item.id === "number";
const getActionableSelectedItems = () =>
  fileList.value.filter((item) => item.isSelected && isRealFileItem(item));
const selectedFiles = computed(() =>
  selectMode.value ? getActionableSelectedItems() : [],
);
...
function getNextImagePath(index: number): string {
  ...
  return target && !target.isPlaceholder && target.file_type === 1
    ? target.file_path
    : "";
}
...
const commentInputText = computed(() => {
  if (selectMode.value) {
    ...
    return selectedItems.every((item) => (item.comments ?? "") === firstComment)
      ? firstComment
      : "";
  }
...
const isDedupPanelOpen = computed(
  () => config.rightPanel.show && config.rightPanel.mode === "dedup",
);
const isInfoPanelOpen = computed(
  () => config.rightPanel.show && config.rightPanel.mode === "info",
);
...
const activeRightPanelWidth = computed(() =>
  Number(config.rightPanel.width || 360),
);
const shouldShowRightPanel = computed(
  () => config.rightPanel.show || selectMode.value,
);
...
function clampRightPanelWidth(width: number) {
  return Math.round(
    Math.min(Math.max(width, RIGHT_PANEL_MIN_WIDTH), getRightPanelMaxWidth()),
  );
}
...
function migrateRightPanelWidthToPixels() {
  ...
  if (rawWidth <= 100) {
    config.rightPanel.width = clampRightPanelWidth(
      (rawWidth / 100) * layoutWidth,
    );
    return;
  }
...
const openTrashMsgbox = (
  ...
  dedupDeleteFileIds.value = Array.isArray(fileIds)
    ? [...new Set(fileIds)]
    : [];
...
const trashMsgboxTitle = computed(() => {
  ...
  return isDedupTrash.value || selectMode.value
    ? localeMsg.value.msgbox.trash_files.title
    : localeMsg.value.msgbox.trash_file.title;
});
...
const trashMsgboxOkText = computed(() => {
  ...
  return isDedupTrash.value || selectMode.value
    ? localeMsg.value.msgbox.trash_files.ok
    : localeMsg.value.msgbox.trash_file.ok;
});

const trashMsgboxMessage = computed(() => {
  const deleteCount = isDedupTrash.value
    ? dedupDeleteFileIds.value.length
    : selectedCount.value;
  const base = permanentDeleteChecked.value
    ? isDedupTrash.value || selectMode.value
      ? localeMsg.value.msgbox.permanent_delete.files_content.replace(
          "{count}",
          deleteCount.toLocaleString(),
        )
      : localeMsg.value.msgbox.permanent_delete.file_content.replace(
          "{file}",
          fileList.value[selectedItemIndex.value]?.name || "",
        )
    : isDedupTrash.value || selectMode.value
      ? localeMsg.value.msgbox.trash_files.content.replace(
          "{count}",
          deleteCount.toLocaleString(),
        )
      : localeMsg.value.msgbox.trash_file.content.replace(
          "{file}",
          fileList.value[selectedItemIndex.value]?.name || "",
        );
  if (dedupReclaimBytes.value <= 0 || !(isDedupTrash.value || selectMode.value))
    return base;
...
const isGeometryGridStyle = computed(
  () => config.settings.grid.style === 2 || config.settings.grid.style === 3,
);
...
const gap = 11; // Gap between items (must match GridView)
...
const shortcut = (actionId: ShortcutActionId) =>
  getShortcutLabel(actionId, shortcutPlatform);
...
function getMatchedRating(event: KeyboardEvent) {
  const match = ratingActions.find(({ actionId }) =>
    matchesShortcut(actionId, event, shortcutPlatform),
  );
...
function hasExternalDomDrop(event: DragEvent) {
  ...
  return (
    !isContentInternalDrag.value &&
    !!dt &&
    (dt.files.length > 0 || !!getExternalDropUrl(dt))
  );
}
...
function isInternalReorderActive() {
  return (
    uiStore.isInputActive("ManageLibraries") ||
    uiStore.isInputActive("AlbumList-edit")
  );
}
...
const isFilmstripVertical = computed(
  () =>
    config.settings.grid.showFilmStrip &&
    config.settings.grid.previewPosition >= 2,
);
...
const showWelcomeContent = computed(
  () => props.libraryEmpty && libraryChecked.value,
);
          ...
          case 0:
            switch (libConfig.album.id) {
              case 0:
                return IconPhotoAll;
              default:
                return libConfig.album.selected ? IconPhotoAll : IconFolder;
            }
          case 1:
            switch (libConfig.favorite.folderId) {
              case 0:
                return IconHeart;
              case 1:
                return IconFolderFavorite;
              default:
                return IconFolderFavorite;
            }
          case 2:
            return IconPhotoSearch;
          case 3:
            return config.calendar.isMonthly
              ? IconCalendarMonth
              : IconCalendarDay;
          case 4:
            return (libConfig.tag as any).tab === "smart"
              ? IconSmartTag
              : IconTag;
          case 5:
...
function toggleKeyboardSelection(direction: "prev" | "next") {
  ...
  if (currentIndex < 0 || nextIndex < 0 || nextIndex >= fileList.value.length)
    return false;
...
async function handleDateGroupSelect({
  startIndex,
  endIndex,
  selected,
}: {
  ...
  const start = Math.max(
    0,
    Math.min(Number(startIndex || 0), fileList.value.length),
  );
  const end = Math.max(
    start,
    Math.min(Number(endIndex || start), fileList.value.length),
  );
  if (start >= end) return;

  const needsLoad = fileList.value
    .slice(start, end)
    .some((f: any) => f?.isPlaceholder);
  if (needsLoad) {
...
function clickRename() {
  renamingFileName.value = extractFileName(
    fileList.value[selectedItemIndex.value].name,
  );
...
async function clickSetAlbumCover() {
  ...
  if (file && albumId) {
    try {
      ...
      await tauriEmit("album-cover-changed", {
        albumId: albumId,
        fileId: file.id,
      });
...
function requestNavigate(direction: "prev" | "next") {
  checkUnsavedChanges(() => {
    const viewer = showQuickView.value
      ? quickViewMediaRef.value
      : config.settings.grid.showFilmStrip
        ? filmStripMediaRef.value
        : null;
  ...
  if (!config.settings.grid.showFilmStrip) {
    ...
    if (maxScrollTop <= 0) {
      ...
      const maxIndex = Math.max(
        1,
        totalFileCount.value - visibleItemCount.value,
      );
...
function handleGridScroll(event: any) {
  if (event && event.target) {
    updateScrollPosition(event.target.scrollTop, event.target.scrollHeight);
  }
}
    ...
    checkUnsavedChanges(() => {
      if (gridViewRef.value) {
        if (usesGeometryNavigation.value) {
          ...
          const nextIndex = gridViewRef.value.getNextItemIndex(
            selectedItemIndex.value,
            "down",
          );
          selectedItemIndex.value =
            nextIndex !== -1 ? nextIndex : selectedItemIndex.value;
        } else {
          selectedItemIndex.value = Math.min(
            selectedItemIndex.value + gridViewRef.value.getColumnCount(),
...
function handleLocalKeyDown(event: KeyboardEvent) {
  ...
  if (
    selectMode.value &&
    matchesShortcut("file.invertSelection", event, shortcutPlatform)
  ) {
    event.preventDefault();
  ...
  if (
    (showQuickView.value || config.settings.grid.showFilmStrip) &&
    matchesShortcut("slideshow.toggle", event, shortcutPlatform)
  ) {
    event.preventDefault();
    toggleSlideShow();
  ...
  if (
    getActivePreviewMode() !== "none" &&
    matchesShortcut("view.zoomIn", event, shortcutPlatform) &&
    event.key === "="
  ) {
    event.preventDefault();
  ...
  if (
    getActivePreviewMode() !== "none" &&
    matchesShortcut("view.zoomOut", event, shortcutPlatform) &&
    event.key === "-"
  ) {
    event.preventDefault();
  ...
  if (
    ...
    event.preventDefault();
    config.settings.grid.size = Math.min(
      360,
      Number(config.settings.grid.size || 160) + 10,
    );
    return;
  }
  ...
  if (
    ...
    if (!isContentInteractionActive()) return;
    event.preventDefault();
    config.settings.grid.size = Math.max(
      120,
      Number(config.settings.grid.size || 160) - 10,
    );
    return;
  ...
  if (
    matchesShortcut("view.quickPreview", event, shortcutPlatform) &&
    event.key === "Enter"
  ) {
  ...
  if (
    event.shiftKey &&
    selectionDirection &&
    toggleKeyboardSelection(selectionDirection)
  ) {
    return;
  }
  ...
  if (
    ...
    (isFilmstrip && event.key === "ArrowDown")
  ) {
    requestNavigate("next");
  } else if (
    event.key === "ArrowLeft" ||
    (isFilmstrip && event.key === "ArrowUp")
  ) {
...
const indexRecoveryTitle = computed(
  () => localeMsg.value.search.index.recovery.title,
);
const indexRecoveryOkText = computed(
  () => localeMsg.value.search.index.recovery.continue,
);
const indexRecoverySkipLabel = computed(
  () => localeMsg.value.search.index.recovery.skip_once,
);
const indexRecoveryFileLabel = computed(
  () => localeMsg.value.search.index.recovery.file_label,
);
...
async function processNextAlbum(
  ...
  if (libConfig.index.albumQueue.length > 0) {
    ...
    if (album) {
      ...
      libConfig.index.pausedAlbumIds = (
        libConfig.index.pausedAlbumIds as any[]
      ).filter((id) => Number(id) !== Number(albumId));
...
const isIndexing = computed(() => {
  return (
    config.main.sidebarIndex === 0 && // Album mode
    !!libConfig.album.id &&
    libConfig.album.id > 0 && // Valid album
...
const isAlbumPaused = (albumId: number | null | undefined) =>
  (libConfig.index.pausedAlbumIds as any[]).some(
...
const activeScanningAlbumId = computed(() =>
  Number(libConfig.index.albumQueue[0] || 0),
);
const suppressNextIndexingIdleRefresh = ref(false);
const selectedAlbumIdForStatusBar = computed(() =>
  Number(libConfig.album.id || 0),
);
const selectedAlbumScanState = computed(() =>
  getAlbumScanState({
...
const selectedAlbumScanIcon = computed(() =>
  getAlbumScanIcon(selectedAlbumScanState.value),
);
const selectedAlbumScanAnimating = computed(() =>
  shouldAnimateAlbumScanIcon(selectedAlbumScanState.value),
);
...
const statusBarScanMode = computed<
  "none" | "current" | "waiting" | "background" | "paused"
>(() => {
...
const statusBarShowUpdateIcon = computed(
  () =>
    selectedAlbumScanIcon.value !== "none" ||
    statusBarScanMode.value === "background",
);
const statusBarUpdateIcon = computed<"update" | "dot">(() =>
  statusBarScanMode.value === "background"
    ? "update"
    : selectedAlbumScanIcon.value === "dot"
      ? "dot"
      : "update",
);
...
const statusBarScanText = computed(() => {
  ...
  if (phase === "preparing_previews") {
    return (
      localeMsg.value.search.index.preparing_previews ||
      "Generating previews... {count} / {total}"
    )
  ...
  if (phase === "preparing_search") {
    return (
      localeMsg.value.search.index.preparing_search ||
      "Preparing search... {count} / {total}"
    )
  ...
  if (phase === "complete") {
    if (Number(libConfig.index.failed || 0) > 0) {
      return (
        localeMsg.value.search.index.complete_with_issues ||
        "Scan complete with {count} issues"
      ).replace(
        "{count}",
        Number(libConfig.index.failed || 0).toLocaleString(),
      );
    }
  ...
  return (
    localeMsg.value.search.index.discovering ||
    "Scanning library... {count} / {total}"
  )
...
function queueScanVisiblePrefetch(startIndex: number, endIndex: number) {
  ...
  scanVisiblePrefetchTimer = setTimeout(() => {
    ...
    fetchDataRange(
      scanVisiblePrefetchStart.value,
      scanVisiblePrefetchEnd.value,
    );
  }, 80);
}
...
function queueScanStreamingPull(albumId: number, current: number) {
  ...
  scanStreamQueuedIndexed.value = Math.max(
    scanStreamQueuedIndexed.value,
    Number(current || 0),
  );
  ...
  scanStreamFlushTimer = setTimeout(async () => {
    ...
    if (
      ...
      queueScanStreamingPull(
        currentAlbumId,
        Number(libConfig.index.discovered || 0),
      );
    }
  }, 300);
}
...
async function cancelIndexing() {
  ...
  const index = normalizedAlbumQueue.value.findIndex(
    (id) => id === Number(albumId || 0),
  );
  if (index === -1) return;
...
onMounted(async () => {
  ...
  pendingInitialSelectedIndex = Number(
    appConfig?.last_selected_item_index ?? -1,
  );
  ...
  unlistenLibraryTotalRefreshed = await listen(
    ...
    if (dt.files.length > 0) {
      ...
      for (const file of dt.files) {
        ...
        try {
          ...
          const result = await importFileBytes(
            bytes,
            file.name,
            folderId,
            folderPath,
  ...
  unlistenImageViewer = await listen(
    "message-from-image-viewer",
    async (event) => {
      ...
      switch (message) {
        case "request-file-at-index":
          ...
          const pane =
            (event.payload as any).pane === "right" ? "right" : "left";
  ...
  unlistenImageEditor = await listen(
    "message-from-image-editor",
    async (event: any) => {
        ...
        try {
          ...
          if (editorWindow) {
            try {
              ...
              console.error(
                "Failed to destroy ImageEditor window from parent:",
                error,
              );
            }
          }
  ...
  unlistenIndexProgress = await listenIndexProgress(async (event: any) => {
    ...
    if (
      libConfig.index.albumQueue.length > 0 &&
      libConfig.index.albumQueue[0] === album_id
    ) {
      libConfig.index.phase = String(
        phase || libConfig.index.phase || "discovering",
      );
  ...
  unlistenThumbnailReady = await listen(
    "thumbnail_ready",
    async (event: any) => {
      ...
      const loadedFiles = fileList.value.filter(
        (file: any) =>
          !file?.isPlaceholder && readyIds.has(Number(file.id || 0)),
      );
...
function restoreInitialSelectionIfNeeded() {
  if (
    hasRestoredInitialSelection ||
    pendingInitialSelectedIndex < 0 ||
...
watch(
  () => libConfig.index.status,
  (newStatus) => {
    if (
      newStatus === 1 &&
      libConfig.index.albumQueue.length > 0 &&
      !isRecoveryInProgress.value
    ) {
      processNextAlbum();
    }
  },
...
watch(
  () => libConfig.index.albumQueue.length,
  (newLength) => {
...
watch(
  () => [
    ...
    libConfig.album.folderPath,
    libConfig.album.selected, // album
    (libConfig.favorite as any).tab,
    libConfig.favorite.albumId,
    libConfig.favorite.folderId,
    libConfig.favorite.folderPath,
    libConfig.favorite.rating, // favorite files and rating
    libConfig.search.fileName,
    config.search.fileType,
    config.search.sortType,
    config.search.sortOrder, // search and sort
    ...
    libConfig.calendar.date, // calendar
    libConfig.tag.id,
    libConfig.tag.smartId,
    (libConfig.tag as any).tab, // tag
    libConfig.location.admin1,
    libConfig.location.name, // location
    libConfig.camera.make,
    libConfig.camera.model, // camera
    (libConfig.camera as any).tab,
    (libConfig.camera as any).lensMake,
    (libConfig.camera as any).lensModel, // lens
  ],
  () => {
...
watch(
  ...
  () => {
    ...
    selectedSize.value = selectedItems.reduce(
...
async function fetchDataRange(start: number, end: number, reverse = false) {
  ...
  for (
    let i = chunkStartIdx;
    reverse ? i >= chunkEndIdx : i <= chunkEndIdx;
    reverse ? i-- : i++
  ) {
  ...
  try {
    for (
      ...
      for (let i = 0; i < loadedFiles.length; i++) {
        ...
        if (targetIndex >= fileList.value.length || targetIndex >= cappedTarget)
          break;
...
function handleVisibleRangeUpdate({
  startIndex,
  endIndex,
}: {
  startIndex: number;
  ...
  if (
    lastVisibleRange.start === startIndex &&
    lastVisibleRange.end === endIndex
  ) {
    return;
  }
...
async function getFileList(
  ...
  try {
    ...
    if (result) {
      ...
      fileList.value = Array.from({ length: totalFileCount.value }).map(
        (_, i) => ({
          id: "ph-" + i,
          isPlaceholder: true,
          name: "",
...
async function getImageSearchFileList(
  ...
  currentImageSearchParams.value = {
    ...
    threshold:
      thresholdOverride ??
      config.imageSearchThresholds[config.settings.imageSearch.thresholdIndex],
...
async function updateContent(force = false) {
  ...
  if (newIndex === 0) {
        ...
        if (album) {
          if (libConfig.album.selected) {
            ...
            getFolderSearchExcluded(folderPath).then((excluded) => {
              if (requestId === currentContentRequestId)
                isCurrentFolderExcluded.value = !!excluded;
            });
            ...
            if (folderId > 0 && folderPath) {
              ...
              const syncPromise =
                existing ??
                syncAlbumFolderMtimes(album.id, folderId, folderPath);
    ...
    if (libConfig.favorite.folderId === null) {
      ...
      if (libConfig.favorite.rating === 0) {
      ...
      } else if (libConfig.favorite.folderId === 0) {
        // favorite files
        ...
        getAlbum(libConfig.favorite.albumId).then((album) => {
          if (requestId !== currentContentRequestId) return;
          if (album) {
            contentTitle.value = formatFolderBreadcrumb(
              libConfig.favorite.folderPath || "",
              album.path,
            );
            getFileList(
              { searchAllSubfolders: libConfig.favorite.folderPath || "" },
              requestId,
            );
          } else {
            contentTitle.value = "";
    ...
    if (libConfig.search.searchType === 0) {
      ...
      if (libConfig.search.searchText) {
        contentTitle.value =
          localeMsg.value.search.search_images +
          " - " +
          libConfig.search.searchText;
      ...
      if (index >= 0 && index < libConfig.search.similarImageHistory.length) {
        const file = await getFileInfo(
          libConfig.search.similarImageHistory[index],
        );
        contentTitle.value =
          localeMsg.value.search.similar_images +
          " - " +
          shortenFilename(file.name, 32);
      ...
      if (libConfig.search.fileName) {
        contentTitle.value =
          localeMsg.value.search.filename_search +
          " - " +
          libConfig.search.fileName;
        getFileList(
          {
            searchFileName: libConfig.search.fileName,
            sortType: 3,
            sortOrder: 0,
          },
          requestId,
    ...
    if (libConfig.calendar.year === null) {
      contentTitle.value = "";
      showEmptyContent(requestId);
    } else if (libConfig.calendar.year === -1) {
      // on this day
      contentTitle.value = localeMsg.value.calendar.on_this_day;
      getFileList({ startDate: -1, endDate: -1 }, requestId);
    } else {
      if (libConfig.calendar.month === -1) {
        // yearly
        contentTitle.value = formatDate(
          libConfig.calendar.year!,
          1,
          1,
          localeMsg.value.format.year,
        );
      } else if (libConfig.calendar.date === -1) {
        // monthly
        contentTitle.value = formatDate(
          libConfig.calendar.year!,
          libConfig.calendar.month!,
          1,
          localeMsg.value.format.month,
        );
      } else {
        // daily
        contentTitle.value = formatDate(
          ...
          localeMsg.value.format.date_long,
        );
      }
    ...
    if (tagTab === "smart") {
      ...
      if (!smartId) {
        ...
        const smartTagLabel =
          localeMsg.value.tag.smart_items?.[smartTag.id] || smartTag.id;
    ...
    if (libConfig.person.id === null) {
      ...
      contentTitle.value =
        libConfig.person.name || `${localeMsg.value.sidebar.people}`;
    ...
    if (libConfig.location.admin1 === null) {
      ...
      if (libConfig.location.name) {
        ...
        getFileList(
          {
            locationAdmin1: libConfig.location.admin1,
            locationName: libConfig.location.name,
          },
          requestId,
        );
      } else {
    ...
    if ((libConfig.camera as any).tab === "lens") {
      ...
      if (libConfig.camera.model) {
        ...
        getFileList(
          { make: libConfig.camera.make, model: libConfig.camera.model },
          requestId,
        );
      } else {
...
function enterSimilarSearchMode(file: any) {
  ...
  contentTitle.value =
    localeMsg.value.search.similar_images + " - " + file.name;

  // Update similar image search history
  const existingIndex = (
    libConfig.search.similarImageHistory as any[]
  ).findIndex((item) => item === file.id);
  ...
  if (existingIndex !== -1) {
    ...
    if (
      libConfig.search.similarImageHistory.length >
      config.search.maxSearchHistory
    ) {
...
function enterAlbumPreviewMode(file: any, targetFolderPath?: string) {
  ...
  if (
    ...
    currentQueryParams.value.searchFolder === folderPath
  )
    return;
...
const refreshAffectedAlbums = async (
  albumIds: Array<number | null | undefined>,
) => {
  const uniqueAlbumIds = Array.from(
    new Set(albumIds.map((id) => Number(id || 0)).filter((id) => id > 0)),
  );
...
const onMoveTo = async () => {
  ...
  const destLabel =
    getFolderName(libConfig.destFolder.folderPath || "") ||
    libConfig.destFolder.folderPath ||
    "";
  let successCount = 0;
  ...
  if (destFolderId <= 0 && libConfig.destFolder.folderPath) {
    ...
    if (destAlbumId > 0) {
      const resolved = await selectFolder(
        destAlbumId,
        libConfig.destFolder.folderPath,
      );
      if (resolved?.id) {
  ...
  if (selectMode.value && selectedCount.value > 0) {
    ...
    const sourceLabel = t("toolbar.filter.select_count", {
      count: selectedCount.value.toLocaleString(),
    });
    const moves = getActionableSelectedItems().map(async (item) => {
    ...
    if (successIds.length > 0) {
      ...
      selectedItemIndex.value =
        fileList.value.length > 0
          ...
          : -1;
      toast.success(
        t("msgbox.move_to.success", { source: sourceLabel, dest: destLabel }),
      );
    }
  ...
  if (successCount === 0) {
    const sourceLabel = selectMode.value
      ...
      : fileList.value[selectedItemIndex.value]?.name || "";
    toast.error(
      t("msgbox.move_to.error", { source: sourceLabel, dest: destLabel }),
    );
  }
...
const onCopyTo = async () => {
  ...
  if (selectMode.value && selectedCount.value > 0) {
    ...
    const copies = getActionableSelectedItems().map(async (item) => {
    ...
    if (successCount > 0) {
      toast.success(
        t("msgbox.copy_to.success", { source: sourceLabel, dest: destLabel }),
      );
    }
  } else if (selectedItemIndex.value >= 0) {
    // single select mode
  ...
  if (successCount === 0) {
    const sourceLabel = selectMode.value
      ...
      : fileList.value[selectedItemIndex.value]?.name || "";
    toast.error(
      t("msgbox.copy_to.error", { source: sourceLabel, dest: destLabel }),
    );
  }
...
const onExportTo = async () => {
  ...
  const files =
    selectMode.value && selectedCount.value > 0
  ...
  if (successCount > 0) {
    toast.success(
      t("msgbox.export_to.success", { source: sourceLabel, dest: destLabel }),
    );
  } else {
    toast.error(
      t("msgbox.export_to.error", { source: sourceLabel, dest: destLabel }),
    );
  }
};
...
const getDeleteFileSuccessMessage = (fileName: string, permanently: boolean) =>
  permanently
    ? localeMsg.value.msgbox.permanent_delete.file_success.replace(
        "{file}",
        fileName,
      )
...
const getDeleteFilesSuccessMessage = (count: number, permanently: boolean) =>
  permanently
    ? localeMsg.value.msgbox.permanent_delete.files_success.replace(
        "{count}",
        count.toLocaleString(),
      )
    : localeMsg.value.msgbox.trash_files.success.replace(
        "{count}",
        count.toLocaleString(),
      );
...
const onTrashFile = async () => {
  ...
  const shouldAdvanceDedup =
    isDedupPanelOpen.value && !!dedupTrashGroupKey.value;
  const preDeleteGroups = shouldAdvanceDedup
    ? buildDuplicateGroups(fileList.value)
    : [];
  ...
  try {
    if (dedupDeleteFileIds.value.length > 0) {
      ...
      if (failedDeleteCount > 0 && deletedFileIds.length === 0) {
        throw new Error(
          `Failed to ${permanently ? "permanently delete" : "trash"} dedup files`,
        );
      }
      ...
      selectedItemIndex.value =
        fileList.value.length > 0
          ? Math.min(selectedItemIndex.value, fileList.value.length - 1)
          : -1;
    } else if (selectMode.value && selectedCount.value > 0) {
      // multi-select mode
      const selectedItems = getActionableSelectedItems();
      const deleteResults = await Promise.allSettled(
        selectedItems.map(async (item) => {
      ...
      if (failedDeleteCount > 0 && deletedItems.length === 0) {
        throw new Error(
          `Failed to ${permanently ? "permanently delete" : "trash"} selected files`,
        );
      }
      ...
      const deletedFileName =
        fileList.value[selectedItemIndex.value]?.name || "";
      affectedAlbumIds.add(
        Number(fileList.value[selectedItemIndex.value].album_id || 0),
      );
      deletedFileIds.push(fileList.value[selectedItemIndex.value].id);
      await deleteFileAlways(
        fileList.value[selectedItemIndex.value],
        permanently,
      );
      removeFromFileList(selectedItemIndex.value);
    ...
    if (shouldAdvanceDedup) {
      ...
      if (postDeleteGroups.length > 0) {
        const previousIndex = preDeleteGroups.findIndex(
          (group) => group.key === currentDedupGroupKey,
        );
        ...
        const availableKeys = new Set(
          postDeleteGroups.map((group) => group.key),
        );
        const nextKey =
          orderedNextKeys.find((key) => availableKeys.has(key)) ||
          postDeleteGroups[0].key;
        const nextGroup = postDeleteGroups.find(
          (group) => group.key === nextKey,
        );
    ...
    console.error(
      `Failed to ${permanently ? "permanently delete" : "trash"} file(s):`,
      error,
    );
    toast.error(
...
async function deleteFileAlways(file: any, permanently = false) {
  ...
  if (deletedFile) {
    console.log(
      `clickDeleteFile - ${permanently ? "permanently deleted" : "trashed"} file:`,
      file.file_path,
    );
  } else {
    throw new Error(
      `Failed to ${permanently ? "permanently delete" : "trash"} file: ${file.file_path}`,
    );
  }
}
...
function getNextVideoThumbnailRefreshPercent(file: any) {
  ...
  const percent =
    VIDEO_THUMB_REFRESH_PERCENTS[
      currentIndex % VIDEO_THUMB_REFRESH_PERCENTS.length
    ];
  videoThumbRefreshIndex.set(
    fileId,
    (currentIndex + 1) % VIDEO_THUMB_REFRESH_PERCENTS.length,
  );
...
const updateThumbForFile = async (file: any) => {
  ...
  if (thumb) {
    if (thumb.error_code === 0 || thumb.error_code === 2) {
      file.thumbnail = getThumbnailDataUrl(
        thumb,
...
const syncFileMetaToImageViewer = async (
  fileId: number,
...
const insertIndexedFileIntoList = async (indexedFile: any) => {
  const position = await getQueryFilePosition(
    currentQueryParams.value,
    indexedFile.id,
  );
  ...
  const existingIndex = fileList.value.findIndex((file: any) => {
    ...
    const samePath =
      ...
      file.file_path === indexedFile.file_path;
...
const toggleSelectModeFavorite = async () => {
  ...
  const selectedItems = getActionableSelectedItems();
  if (selectedItems.length === 0) return;
  const shouldFavorite = !selectedItems.every((item) =>
...
function scheduleNextSlide() {
  ...
  const interval =
    getSlideShowInterval(config.settings.slideShowInterval) * 1000;
...
const clickRotate = async () => {
  ...
  if (selectedItemIndex.value >= 0) {
    ...
    tauriEmit("message-from-content", {
      message: "rotate",
      fileId: fileList.value[selectedItemIndex.value].id,
    });

    // update the rotate status in the database
    setFileRotate(
      fileList.value[selectedItemIndex.value].id,
      fileList.value[selectedItemIndex.value].rotate,
    );
...
const selectAllInCurrentList = async () => {
  ...
  const selectionCap = isOverLimit
    ? selectionMaxFiles.value
...
const handleSelectMode = (value: any, options: { notify?: boolean } = {}) => {
  ...
  if (!selectMode.value) {
    ...
    if (fileList.value.length > 0) {
      const fallbackIndex = fileList.value.findIndex((item) =>
        isRealFileItem(item),
      );
      const targetIndex =
        selectedItemIndex.value >= 0 &&
...
const fileTypeSelectedValues = computed(() => {
  ...
  if (mask === 0) return [0];
  return [FILE_TYPE_IMAGE, FILE_TYPE_RAW, FILE_TYPE_VIDEO].filter(
...
const handleFileTypeSelect = (values: any[]) => {
  ...
  const nextValues = (Array.isArray(values) ? values : []).map((value) =>
    Number(value),
  );
...
const handleDedupCompareGroup = async (groupId: string, keepFileId: number) => {
  ...
  await openImageViewer(leftIndex, true, false, {
    rightIndex,
    forceSplit: true,
  });
};
  ...
  for (const fileId of fileIds) {
    ...
    if (index !== -1) {
      ...
      getFileHasTags(fileId).then(async (hasTags) => {
        ...
        syncFileMetaToImageViewer(fileId, {
          has_tags: newFile.has_tags,
          tags: newFile.tags,
        });
      });
    }
  ...
  const processBatch = async (startIndex: number) => {
    ...
    try {
      ...
      for (let j = 0; j < batchFiles.length; j++) {
        applyThumbToFile(
          batchFiles[j],
          Array.isArray(thumbs) ? thumbs[j] : null,
        );
      }
    } catch (error) {
  ...
  const runWithConcurrencyLimit = async (files: any[]) => {
    ...
    for (let i = offset; i < files.length; i += batchSize) {
      if (requestId !== currentThumbRequestId) {
        console.log(
          `Thumbnail generation cancelled (request ${requestId} superseded by ${currentThumbRequestId})`,
        );
        return;
      }
