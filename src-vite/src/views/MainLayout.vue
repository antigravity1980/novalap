<template>
  <div
    class="main-layout flex h-screen w-screen overflow-hidden bg-base-300 text-base-content select-none font-sans"
    :class="{ 'focus-mode-active': galleryStore.focusMode }"
  >
    <!-- Navigation & Explorer sidebar panel (Unified Left Sidebar) -->
    <aside
      v-if="showSidebar && !galleryStore.focusMode"
    ...
    >
      ...
      <div class="flex-1 overflow-y-auto custom-scrollbar py-3 px-2 space-y-4">
        ...
        <div class="space-y-1">
          <div
            class="px-3 py-1 text-[10px] font-bold text-base-content/40 uppercase tracking-widest"
          >
        ...
        <div class="space-y-1">
          ...
          <div
            v-if="
              ...
              configStore.settings.favorites.length === 0
            "
          ...
          <div
            ...
            :class="[
              activeTab === 'explorer' && navigationStore.currentPath === path
                ? 'bg-primary/15 text-primary font-bold active-nav-item'
                : 'text-base-content/75 hover:bg-base-100/50 hover:text-base-content',
              dragOverFavPath === path
                ? 'bg-secondary/20 text-secondary border border-dashed border-secondary/50'
                : '',
            ]"
          ...
          >
            <span
              class="text-sm shrink-0 flex items-center justify-center w-4 h-4"
              >⭐</span
            >
        ...
        <div class="space-y-1 relative">
          <div
            class="px-3 py-1 text-[10px] font-bold text-base-content/40 uppercase tracking-widest flex items-center justify-between"
          >
          ...
          <!-- Dropdown container -->
          <div
            v-if="showDrivesDropdown"
            class="absolute left-3 top-6 bg-base-300 border border-neutral/30 rounded-lg shadow-xl py-2 px-3 z-50 text-xs text-base-content normal-case font-semibold w-48 space-y-1"
          >
            <div
              class="text-[10px] font-bold text-base-content/40 uppercase tracking-wider mb-1.5 pb-1 border-b border-base-content/5"
            >
              Показать диски:
            </div>
            <label
              v-for="d in navigationStore.drives"
              :key="d.path"
              class="flex items-center gap-2 cursor-pointer hover:bg-base-100/40 p-1 rounded"
            >
              <input
                type="checkbox"
      ...
      <div
        class="p-2 border-t border-neutral/20 shrink-0 bg-base-200/50 flex items-center justify-between"
      >
        <button
    ...
    >
      ...
      >
        <div
          class="flex items-center gap-1 overflow-hidden h-full pt-1.5"
          @mousedown.stop
        >
          <!-- Loop through tabs -->
          <div
            ...
            :class="
              t.id === activeTabId
                ? 'bg-base-100 text-base-content/85'
                : 'bg-base-300/40 text-base-content/50 hover:bg-base-100/30'
            "
      ...
      >
          ...
          >
            ...
            >
              <path
                ...
                d="M9 4.5v15m6-15v15m-12-15h18c.621 0 1.125.504 1.125 1.125v15c0 .621-.504 1.125-1.125 1.125H3.75A1.125 1.125 0 012.625 18V5.625c0-.621.504-1.125 1.125-1.125z"
              />
            </svg>
          </button>
        ...
        <div
          class="flex-1 flex items-center border border-neutral/30 rounded bg-base-100 px-3 py-1 text-xs select-none max-w-3xl h-8"
        >
          <span class="mr-2 opacity-65 text-sm">💻</span>

          <div
            v-if="!isEditingPath"
            class="flex-1 flex items-center overflow-x-auto no-scrollbar whitespace-nowrap gap-1 cursor-text h-full"
          ...
          >
            ...
            <div
              ...
              :key="crumb.path"
              class="flex items-center gap-1 text-base-content/85"
            >
              <span
                class="hover:text-primary transition-colors cursor-pointer"
              ...
              >
              <span
                class="text-base-content/40 text-[9px] font-bold"
        ...
        <div class="relative w-56 shrink-0 flex items-center">
          ...
          />
          <IconSearch
            class="absolute left-2.5 text-base-content/40 w-3.5 h-3.5"
          />
        </div>
      </header>
      ...
      >
        <div class="flex items-center gap-1 px-3 shrink-0">
          ...
          <button
            ...
            @dragover.prevent="compareDropActive = true"
            @dragenter.prevent="compareDropActive = true"
            @dragleave.prevent="compareDropActive = false"
          ...
          >
            ...
            <span
              v-if="
                ...
                galleryStore.compareStack.length > 0
              "
              class="badge badge-primary badge-xs"
              >{{ galleryStore.compareStack.length }}</span
            >
          </button>
          ...
          <button
            ...
            :class="
              galleryStore.selectionMode
                ? 'win11-btn-active-selection'
                : 'font-semibold'
            "
          ...
          <button
            v-if="galleryStore.selectionMode"
            class="win11-btn text-xs font-semibold flex items-center gap-1.5 transition-all duration-150 hover:border-primary/40"
            :class="{
              'win11-btn-active-selection': galleryStore.focusMode,
            }"
            @click="galleryStore.toggleFocusMode()"
            title="Развернуть галерею на весь экран (только миниатюры + избранное). Выход: ESC"
          >
            <svg
              class="w-3.5 h-3.5"
              viewBox="0 0 24 24"
          ...
          <div class="dropdown">
            ...
            <ul
              tabindex="0"
              class="dropdown-content menu p-1.5 shadow-2xl bg-base-300 border border-neutral/30 rounded-lg w-52 z-30 text-xs mt-1"
            >
              <li>
                <a
              ...
              <li>
                ...
                >
                  <span
                    >{{ $t("toolbar.filter.sort_order_options[0]") }} (↑)</span
                  >
              ...
              <li>
                ...
                >
                  <span
                    >{{ $t("toolbar.filter.sort_order_options[1]") }} (↓)</span
                  >
          ...
          <div class="dropdown">
            ...
            >
              <li>
                <a
                  :class="{ active: galleryStore.groupBy === 'none' }"
                  @click="
                    galleryStore.groupBy = 'none';
                    onGroupChange();
                  "
                  class="flex justify-between items-center"
                >
              ...
              <li>
                <a
                  :class="{ active: galleryStore.groupBy === 'type' }"
                  @click="
                    galleryStore.groupBy = 'type';
                    onGroupChange();
                  "
                  class="flex justify-between items-center"
                >
              ...
              <li>
                <a
                  :class="{ active: galleryStore.groupBy === 'date' }"
                  @click="
                    galleryStore.groupBy = 'date';
                    onGroupChange();
                  "
                  class="flex justify-between items-center"
                >
              ...
              <li>
                <a
                  :class="{ active: galleryStore.groupBy === 'size' }"
                  @click="
                    galleryStore.groupBy = 'size';
                    onGroupChange();
                  "
                  class="flex justify-between items-center"
                >
              ...
              <li>
                <a
                  :class="{ active: galleryStore.groupBy === 'extension' }"
                  @click="
                    galleryStore.groupBy = 'extension';
                    onGroupChange();
                  "
                  class="flex justify-between items-center"
                >
          ...
          <div class="dropdown">
            ...
            <ul
              tabindex="0"
              class="dropdown-content menu p-1.5 shadow-2xl bg-base-300 border border-neutral/30 rounded-lg w-56 overflow-x-hidden z-30 text-xs mt-1 max-h-96 overflow-y-auto custom-scrollbar"
            >
              <div
                class="px-2 py-1 text-[9px] font-bold text-base-content/40 uppercase"
              >
                {{ $t("explorer.format") }}
              </div>
              <li>
                <a
                  :class="{ active: galleryStore.filters.format === '' }"
                  @click="
                    galleryStore.filters.format = '';
                    onFilterChange();
                  "
                  class="flex justify-between items-center"
                >
              ...
              <li>
                <a
                  :class="{ active: galleryStore.filters.format === 'png' }"
                  @click="
                    galleryStore.filters.format = 'png';
                    onFilterChange();
                  "
                  class="flex justify-between items-center"
                >
              ...
              <li>
                <a
                  :class="{
                    active:
                      galleryStore.filters.format === 'jpg' ||
                      galleryStore.filters.format === 'jpeg',
                  }"
                  @click="
                    galleryStore.filters.format = 'jpg';
                    onFilterChange();
              ...
              <li>
                <a
                  :class="{ active: galleryStore.filters.format === 'webp' }"
                  @click="
                    galleryStore.filters.format = 'webp';
                    onFilterChange();
                  "
                  class="flex justify-between items-center"
                >
              ...
              <li>
                <a
                  :class="{ active: galleryStore.filters.format === 'gif' }"
                  @click="
                    galleryStore.filters.format = 'gif';
                    onFilterChange();
                  "
                  class="flex justify-between items-center"
                >
              ...
              <li>
                <a
                  :class="{ active: galleryStore.filters.format === 'mp4' }"
                  @click="
                    galleryStore.filters.format = 'mp4';
                    onFilterChange();
                  "
                  class="flex justify-between items-center"
                >
              ...
              <li>
                <a
                  :class="{ active: galleryStore.filters.format === 'mkv' }"
                  @click="
                    galleryStore.filters.format = 'mkv';
                    onFilterChange();
                  "
                  class="flex justify-between items-center"
                >
              ...
              <li>
                <a
                  :class="{ active: galleryStore.filters.format === 'webm' }"
                  @click="
                    galleryStore.filters.format = 'webm';
                    onFilterChange();
                  "
                  class="flex justify-between items-center"
                >
              ...
              <li>
                <a
                  :class="{ active: galleryStore.filters.format === 'mov' }"
                  @click="
                    galleryStore.filters.format = 'mov';
                    onFilterChange();
                  "
                  class="flex justify-between items-center"
                >
              ...
              <div
                class="px-2 py-1 text-[9px] font-bold text-base-content/40 uppercase"
              >
                {{ $t("explorer.ai_source") }}
              </div>
              <li>
                <a
                  :class="{ active: galleryStore.filters.aiSource === '' }"
                  @click="
                    galleryStore.filters.aiSource = '';
                    onFilterChange();
                  "
                  class="flex justify-between items-center"
                >
              ...
              <li>
                <a
                  :class="{
                    active: galleryStore.filters.aiSource === 'ComfyUI',
                  }"
                  @click="
                    galleryStore.filters.aiSource = 'ComfyUI';
                    onFilterChange();
                  "
                  class="flex justify-between items-center"
                >
                  <span>ComfyUI</span>
                  <span v-if="galleryStore.filters.aiSource === 'ComfyUI'"
                    >✓</span
                  >
                </a>
              </li>
              <li>
                <a
                  :class="{
                    active: galleryStore.filters.aiSource === 'Midjourney',
                  }"
                  @click="
                    galleryStore.filters.aiSource = 'Midjourney';
                    onFilterChange();
                  "
                  class="flex justify-between items-center"
                >
                  <span>Midjourney</span>
                  <span v-if="galleryStore.filters.aiSource === 'Midjourney'"
                    >✓</span
                  >
                </a>
              </li>
              <li>
                <a
                  :class="{
                    active:
                      galleryStore.filters.aiSource === 'Stable Diffusion',
                  }"
                  @click="
                    galleryStore.filters.aiSource = 'Stable Diffusion';
                    onFilterChange();
                ...
                >
                  ...
                  <span
                    v-if="galleryStore.filters.aiSource === 'Stable Diffusion'"
                    >✓</span
                  >
                </a>
              </li>
              <li>
                <a
                  :class="{
                    active: galleryStore.filters.aiSource === 'Nano Banana',
                  }"
                  @click="
                    galleryStore.filters.aiSource = 'Nano Banana';
                    onFilterChange();
                  "
                  class="flex justify-between items-center"
                >
                  <span>Nano Banana</span>
                  <span v-if="galleryStore.filters.aiSource === 'Nano Banana'"
                    >✓</span
                  >
                </a>
              </li>
              <li>
                <a
                  :class="{
                    active: galleryStore.filters.aiSource === 'GPT Images',
                  }"
                  @click="
                    galleryStore.filters.aiSource = 'GPT Images';
                    onFilterChange();
                  "
                  class="flex justify-between items-center"
                >
                  <span>GPT Images</span>
                  <span v-if="galleryStore.filters.aiSource === 'GPT Images'"
                    >✓</span
                  >
                </a>
              </li>
              <li>
                <a
                  :class="{
                    active: galleryStore.filters.aiSource === 'Grok Image',
                  }"
                  @click="
                    galleryStore.filters.aiSource = 'Grok Image';
                    onFilterChange();
                  "
                  class="flex justify-between items-center"
                >
                  <span>Grok Image</span>
                  <span v-if="galleryStore.filters.aiSource === 'Grok Image'"
                    >✓</span
                  >
                </a>
              </li>
              <li>
                <a
                  :class="{
                    active: galleryStore.filters.aiSource === 'DALL-E',
                  }"
                  @click="
                    galleryStore.filters.aiSource = 'DALL-E';
                    onFilterChange();
                  "
                  class="flex justify-between items-center"
                >
                  <span>DALL-E</span>
                  <span v-if="galleryStore.filters.aiSource === 'DALL-E'"
                    >✓</span
                  >
                </a>
              </li>
              <li>
                <a
                  :class="{
                    active: galleryStore.filters.aiSource === 'Krita AI',
                  }"
                  @click="
                    galleryStore.filters.aiSource = 'Krita AI';
                    onFilterChange();
                  "
                  class="flex justify-between items-center"
                >
                  <span>Krita AI</span>
                  <span v-if="galleryStore.filters.aiSource === 'Krita AI'"
                    >✓</span
                  >
                </a>
              </li>
              <li>
                <a
                  :class="{
                    active: galleryStore.filters.aiSource === 'Unknown',
                  }"
                  @click="
                    galleryStore.filters.aiSource = 'Unknown';
                    onFilterChange();
                  "
                  class="flex justify-between items-center"
                >
                  <span>{{ $t("explorer.unknown_source") }}</span>
                  <span v-if="galleryStore.filters.aiSource === 'Unknown'"
                    >✓</span
                  >
                </a>
              </li>
        ...
        <div class="flex items-center gap-1.5 px-3 shrink-0">
          ...
          >
            <span
              v-if="configStore.settings.appearance === 0"
      ...
      >
        <div class="flex items-center gap-3">
          ...
          <span
            >Папок: {{ totalFoldersCount }}, файлов: {{ totalFilesCount }}</span
          >
        </div>
        <div
          v-if="galleryStore.selectedIds.length > 0"
          class="flex items-center gap-3"
        >
          <span class="text-primary font-medium"
            >Выбрано элементов: {{ galleryStore.selectedIds.length }}</span
          >
          <span class="opacity-40">|</span>
    ...
    <div
      v-if="configStore.rightPanel.show && !galleryStore.focusMode"
      class="w-1 shrink-0 cursor-col-resize hover:bg-primary/50 transition-colors z-20"
    ...
    >
      ...
      <div v-if="selectedFile" class="flex flex-col h-full overflow-hidden">
        ...
        <div
          class="tabs tabs-boxed bg-base-300/60 p-1 m-3 rounded-lg shrink-0 flex gap-1"
        >
          <button
        ...
        <!-- Tab contents -->
        <div
          class="flex-1 overflow-y-auto custom-scrollbar px-4 pb-4 space-y-4"
        >
          <!-- A. Details Tab -->
          <div v-if="inspectorTab === 'info'" class="space-y-4">
            <!-- Thumbnail preview -->
            <div
              class="w-full aspect-square rounded-box bg-base-300 border border-neutral/20 overflow-hidden flex items-center justify-center shadow-lg relative group"
            >
              <img
            ...
            <div class="space-y-1">
              <label
                class="text-[10px] text-base-content/40 font-bold uppercase tracking-wider"
                >{{ $t("explorer.file_name") }}</label
              >
            ...
            <div class="grid grid-cols-2 gap-2 text-xs">
              <div
                class="p-2.5 rounded-lg bg-base-100 border border-neutral/20"
              >
                <span
                  class="block text-[9px] text-base-content/40 font-bold uppercase tracking-wider"
                  >{{ $t("explorer.size") }}</span
                >
                <span
                  class="font-semibold text-base-content/85 font-mono mt-0.5 block"
                ...
                >
                  {{
                    selectedFile.resolution
                      ? `${selectedFile.resolution.width}×${selectedFile.resolution.height}`
                      : "—"
                  }}
                </span>
              </div>
            ...
            <div
              v-if="hasComfyWorkflow"
              class="p-2 bg-green-500/10 text-green-600 dark:text-green-400 border border-green-500/20 rounded-lg text-[10px] font-bold uppercase tracking-wider flex items-center justify-center gap-1.5 shadow-sm"
            >
            ...
            <div class="space-y-3 text-xs border-t border-neutral/20 pt-3">
              <div>
                <span
                  class="block text-[9px] text-base-content/40 font-bold uppercase tracking-wider"
                  >{{ $t("explorer.format") }}</span
                >
                <span class="font-medium text-base-content/85 mt-0.5 block">{{
              ...
              <div>
                <span
                  class="block text-[9px] text-base-content/40 font-bold uppercase tracking-wider text-ellipsis overflow-hidden"
                  >{{ $t("explorer.full_path") }}</span
                >
                <span
                  class="font-mono text-[10px] text-base-content/70 mt-0.5 block break-all leading-normal"
            ...
            <div class="flex flex-col gap-1.5 pt-2 border-t border-neutral/20">
              <button
                class="btn btn-ghost btn-sm text-xs font-semibold justify-start hover:bg-base-100/40 rounded-md"
                @click="revealInExplorer(selectedFile.path)"
              >
              ...
              <button
                v-if="isImage(selectedFile)"
                class="btn btn-ghost btn-sm text-xs font-semibold justify-start hover:bg-base-100/40 rounded-md"
                @click="openCrop(selectedFile)"
              >
                ✂️ {{ $t("explorer.quick_crop") }}
              </button>
              <button
                class="btn btn-ghost btn-sm text-xs font-semibold text-error justify-start hover:bg-error/10 rounded-md"
                @click="deleteSingleFile(selectedFile)"
              >
      ...
      <div
        v-else-if="galleryStore.selectedIds.length > 1"
        class="flex flex-col h-full items-center justify-center p-6 text-center text-base-content/50 space-y-4"
      >
        <span class="text-5xl">📦</span>
        <div>
          <h4 class="font-bold text-base-content">
            {{
              $t("explorer.items_selected", {
                count: galleryStore.selectedIds.length,
              })
            }}
          </h4>
          <p class="text-xs mt-1">
            {{
              $t("explorer.total_size", {
                size: formatBytes(totalSelectedSize),
              })
            }}
          </p>
        </div>
        ...
        <div class="flex flex-col gap-2 w-full pt-4">
          <button
            class="btn btn-primary btn-sm text-xs rounded-md"
            :disabled="galleryStore.selectedIds.length < 2"
            @click="openCompare"
          >
          ...
          <button
            v-if="
              galleryStore.compareStack && galleryStore.compareStack.length >= 2
            "
            class="btn btn-secondary btn-sm text-xs rounded-md"
            :class="{ 'ring-2 ring-primary': compareDropActive }"
            @click="openCompareStack"
            @dragover.prevent="compareDropActive = true"
            @dragenter.prevent="compareDropActive = true"
            @dragleave.prevent="compareDropActive = false"
            @drop.prevent.stop="onDropToCompareStack"
          >
            🔍 Сравнить стек ({{ galleryStore.compareStack.length }}/6)
          </button>
          <button
            v-if="
              galleryStore.compareStack && galleryStore.compareStack.length > 0
            "
            class="btn btn-ghost btn-xs text-xs rounded-md"
            @click="galleryStore.clearCompare()"
          >
            Очистить стек сравнения
          </button>
          <button
            class="btn btn-secondary btn-sm text-xs rounded-md"
            @click="batchOperationsVisible = true"
          ...
          <button
            class="btn btn-ghost btn-sm text-xs text-error hover:bg-error/10 rounded-md"
            @click="deleteMultipleSelected"
          >
      ...
      <div
        v-else
        class="flex flex-col h-full items-center justify-center p-6 text-center text-base-content/30 space-y-3"
      >
        <span class="text-4xl">ℹ️</span>
        <div>
          <h4 class="font-semibold text-sm text-base-content/50">
            {{ $t("explorer.no_selection") }}
          </h4>
    ...
    <!-- Compare stack drop-зона / floating action button -->
    <div
      v-if="galleryStore.compareStack && galleryStore.compareStack.length > 0"
      class="fixed bottom-10 right-4 z-40 flex flex-col items-end gap-2"
    >
      <div
        v-if="compareDropActive"
        class="px-3 py-2 rounded-lg bg-primary text-primary-content text-xs font-semibold shadow-2xl border-2 border-primary-content/30 animate-pulse"
      >
        Бросьте файл, чтобы добавить в стек сравнения
      </div>
      <div
        class="flex items-center gap-2 px-3 py-2 rounded-full bg-base-100 border-2 shadow-2xl cursor-pointer transition-colors duration-150"
        :class="compareDropActive ? 'border-primary' : 'border-neutral/30'"
        :title="`Стек сравнения (${galleryStore.compareStack.length}/6) — перетащите файл сюда, чтобы добавить`"
        @dragover.prevent="compareDropActive = true"
        @dragenter.prevent="compareDropActive = true"
        @dragleave.prevent="compareDropActive = false"
        @drop.prevent="onDropToCompareStack"
        @click="openCompareStackIfReady"
      >
        <span class="text-lg leading-none">⚖️</span>
        <div class="flex flex-col items-start leading-tight">
          <span class="text-[11px] font-bold">
            Сравнить ({{ galleryStore.compareStack.length }}/6)
          </span>
          <span class="text-[9px] opacity-70 truncate max-w-[180px]">
            {{ compareStackSummary }}
          </span>
        </div>
        <button
          class="ml-1 w-5 h-5 rounded-full bg-error/20 text-error text-[11px] font-bold hover:bg-error/40"
          title="Очистить стек"
          @click.stop="galleryStore.clearCompare()"
        >
          ✕
        </button>
      </div>
...
import FocusOverlay from "@/modules/gallery/components/FocusOverlay.vue";
...
watch(
  () => navigationStore.navigatedCount,
  () => {
...
// Авто-закрытие CompareView при очистке стека (исправляет рассинхрон FAB ↔ окно).
watch(
  () => galleryStore.compareStack?.length || 0,
  (newLen, oldLen) => {
    if (oldLen > 0 && newLen === 0 && compareVisible.value) {
...
async function resolveCompareFilesFromStack() {
  const stack = galleryStore.compareStack || [];
  if (stack.length < 2) return [];
  const files = await invoke("enrich_entries", {
    paths: stack.map((f) => f.path),
  }).catch(() => []);
  return stack
    .map((entry, idx) => {
      const enrich = Array.isArray(files) ? files[idx] : null;
      return {
        path: entry.path,
        name: entry.name || enrich?.path?.split(/[\\/]/).pop() || entry.path,
        size: enrich?.size ?? 0,
        extension: entry.extension || entry.path.split(".").pop() || "",
        is_dir: false,
      };
    })
    .filter((f) => f.path);
...
const totalFilesCount = computed(() => {
  return galleryStore.files.filter(
    (f) =>
      !(
        f.is_dir === true ||
...
watch(selectedFile, async (newFile) => {
  ...
  if (newFile) {
    ...
    if (isTauri && isImage(newFile)) {
      try {
        const metadata = await invoke("parse_ai_metadata", {
          path: newFile.path,
        });
...
async function createFolder() {
  ...
  const checkNameExists = (n) => {
    return galleryStore.files.some(
...
async function saveFileName() {
  if (
    !selectedFile.value ||
    !renamingState.name ||
    renamingState.name === selectedFile.value.name
  )
    return;
...
async function deleteSingleFile(file) {
  ...
  requestDeleteConfirmation(
    `Переместить ${typeStr} "${file.name}" в корзину?`,
    async () => {
      try {
...
async function deleteMultipleSelected() {
  requestDeleteConfirmation(
    `Переместить ${galleryStore.selectedIds.length} элементов в корзину?`,
    async () => {
      try {
...
function openCompare() {
  ...
  compareFiles.value = galleryStore.selectedIds
    .map((path) => {
...
async function openCompareStack() {
  if ((galleryStore.compareStack?.length || 0) < 2) return;
  compareFiles.value = await resolveCompareFilesFromStack();
  if (compareFiles.value.length < 2) return;
  compareVisible.value = true;
}

const compareDropActive = ref(false);
const compareStackSummary = computed(() => {
  const stack = galleryStore.compareStack || [];
  if (stack.length === 0) return "";
  const names = stack
    .map((f) => (f.name || (f.path ? f.path.split(/[\\/]/).pop() : "")).trim())
    .filter(Boolean);
  if (names.length <= 2) return names.join(" · ");
  return `${names[0]} · ${names[1]} · +${names.length - 2}`;
});

function pushPathsToCompareStack(paths) {
  if (!Array.isArray(paths) || paths.length === 0) return;
  for (const path of paths) {
    if (!path) continue;
    const known = (galleryStore.displayedFiles || []).find(
      (f) => f.path === path,
    );
    const isDir =
      known &&
      (known.is_dir === true ||
        known.file_type === "directory" ||
        known.is_directory === true);
    if (isDir) continue;
    if (galleryStore.isInCompareStack(path)) continue;
    const fallbackName = path.split(/[\\/]/).pop() || path;
    const fallbackExt = (fallbackName.split(".").pop() || "").toLowerCase();
    galleryStore.addToCompare({
      path,
      name: known?.name || fallbackName,
      extension: known?.extension || fallbackExt,
      is_dir: !!isDir,
    });
  }
}

function extractDraggedPaths(event) {
  try {
...
function onDropToCompareStack(event) {
  compareDropActive.value = false;
  const paths = extractDraggedPaths(event);
  pushPathsToCompareStack(paths);
}

function openCompareStackIfReady() {
  if ((galleryStore.compareStack?.length || 0) >= 2) {
    openCompareStack();
  }
}
...
function dragLeftSplitter(e) {
  ...
  configStore.leftPanel.width = Math.max(
    160,
    Math.min(e.clientX - 6, window.innerWidth / 2),
  );
}
...
function dragRightSplitter(e) {
  ...
  configStore.rightPanel.width = Math.max(
    220,
    Math.min(window.innerWidth - e.clientX, window.innerWidth / 2),
  );
}
...
// Выход из focus-режима: закрываем оверлей и возвращаемся к обычному layout.
// selectionMode оставляем включённым — пользователь продолжает выделение.
function onExitFocus() {
  if (galleryStore.focusMode) {
    galleryStore.exitFocusMode();
  }
}

// Tauri fullscreen при входе в focus-режим и выход из него.
// На Windows используем setFullscreen — окно реально разворачивается на весь экран,
// и стандартная панель с кнопками Min/Max/Close скрывается (плюс Chrome-разметка
// скрыта через v-show). На других ОС делаем мягкий fallback — только разметка.
async function applyTauriFullscreen(enabled) {
  if (!isTauri()) return;
  try {
...
watch(
  () => galleryStore.focusMode,
  async (active) => {
    await applyTauriFullscreen(active);
  },
);
...
function toggleTheme() {
  ...
  setTheme(
    nextAppearance,
    nextAppearance === 0
      ? configStore.settings.lightTheme
      : configStore.settings.darkTheme,
  );
}
...
function handleKeyDown(e) {
  ...
  if (e.code === "Space") {
    ...
    if (count === 1) {
      if (
        selectedFile.value &&
        (isImage(selectedFile.value) || isVideo(selectedFile.value))
      ) {
