<template>
  <teleport to="body">
    ...
    >
      ...
      <div
        class="quick-look-header flex items-center justify-between px-6 py-2.5 bg-zinc-950/80 border-b border-white/5 shrink-0 z-10 text-white"
      >
        <!-- File counter and name -->
        <div class="flex items-center gap-3">
          <span
            class="text-white/60 text-xs font-semibold bg-white/10 px-2 py-0.5 rounded"
          >
        ...
        >
          ...
          <div class="flex items-center gap-2">
            <span
              class="text-white/50 text-[11px] font-bold uppercase tracking-wider"
              >{{ $t("viewer.saturation") }}:</span
            >
            <input
              type="range"
              min="0"
            ...
            />
            <span class="text-white/80 font-mono text-xs w-8 text-right"
              >{{ Math.round(saturation * 100) }}%</span
            >
          </div>
      ...
      >
        ...
        >
          ...
          <div
            v-if="imageLoaded"
            class="crop-overlay absolute inset-0 pointer-events-none"
          >
            ...
            <!-- Crop border -->
            <div
              class="absolute border-2 border-primary pointer-events-auto cursor-move"
              :style="{
                left: crop.x + 'px',
                top: crop.y + 'px',
                width: crop.width + 'px',
        ...
        <div v-else class="text-white/50 text-lg">
          {{
            $t("viewer.unsupported_file_type", {
      ...
      <div
        class="file-info-bar py-3 bg-zinc-950/50 text-white/50 text-xs flex items-center justify-center gap-6 shrink-0 border-t border-white/5"
      >
        <span class="font-medium text-white/70">{{ currentFile?.name }}</span>
        <span
          v-if="currentFile?.resolution"
          class="font-mono bg-white/5 px-2 py-0.5 rounded"
        >
          {{ currentFile.resolution.width }}x{{ currentFile.resolution.height }}
        </span>
        <span class="font-mono bg-white/5 px-2 py-0.5 rounded">{{
          formatFileSize(currentFile?.size)
        }}</span>
        <span
          v-if="currentFile?.ai_source"
          class="text-primary/80 font-semibold uppercase tracking-wider bg-primary/10 px-2 py-0.5 rounded"
        >
...
const handles = computed(() => [
  ...
  {
    name: "n",
    style: { left: "50%", top: "-6px", marginLeft: "-6px", cursor: "n-resize" },
  },
  {
    name: "s",
    style: {
  ...
  {
    name: "e",
    style: { right: "-6px", top: "50%", marginTop: "-6px", cursor: "e-resize" },
  },
  {
    name: "w",
    style: { left: "-6px", top: "50%", marginTop: "-6px", cursor: "w-resize" },
  },
]);
